...
export class RTCUserService {
  ...
  private invitationSent: Subject<{
    invitation: RTCInformation;
    source: User;
    dest: User;
  }> = new Subject<{ invitation: RTCInformation; source: User; dest: User }>();

  public onInvitation(
    user: User,
    func: (invitation: RTCInformation, user: User) => any
  ): void {
